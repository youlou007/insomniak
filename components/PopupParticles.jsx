...
const PopupParticles = ({ isVisible, isMobile = false, isNarrow = false, popupWidth = 0 }) => {
  ...
  useEffect(() => {
    ...
    for (let i = 0; i < numberOfParticles; i++) {
      ...
      // Vitesse d'animation
      // En mobile, animation plus lente (deux fois plus lente que la version standard)
      const randomDuration = isMobile 
        ? (8 + Math.random() * 4) // 8-12 secondes pour mobile (plus lent)
    ...
    const interval = isMobile ? 1000 : 1000; // 1000ms en mobile au lieu de 500ms
    const newParticlesCount = isMobile ? 4 : isNarrow ? 3 : 8; // Moins de particules en mobile
    ...
    const intervalId = setInterval(() => {
      if (container && isVisible) {
        ...
        for (let i = 0; i < newParticlesCount; i++) {
          ...
          // Opacité aléatoire légèrement réduite pour mobile
          const opacity = isMobile
            ? 0.1 + Math.random() * 0.4 // Opacité plus faible pour mobile
            : 0.1 + Math.random() * 0.6;
          ...
          // Durée d'animation, plus lente pour mobile
          const randomDuration = isMobile 
            ? (8 + Math.random() * 4) // 8-12 secondes pour mobile (plus lent)
            : (4 + Math.random() * 4); // 4-8 secondes pour desktop
